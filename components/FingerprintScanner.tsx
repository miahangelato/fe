...
const SCANNER_BASE_URL = process.env.NEXT_PUBLIC_SCANNER_BASE_URL;
const CALLBACK_BASE_URL =
  process.env.NEXT_PUBLIC_CALLBACK_BASE_URL ||
...
export default function FingerprintScanner({
  ...
  useEffect(() => {
    ...
    if (waitingForResults && currentSessionId) {
      console.log(
        "🔄 Starting to poll for backend results for session:",
        currentSessionId
      );
      ...
      pollInterval = setInterval(async () => {
        try {
          ...
          const response = await fetch(
            `/api/check-results?sessionId=${currentSessionId}`,
            {
              method: "GET",
              cache: "no-store",
            }
          ...
          if (response.ok) {
            ...
            if (data.hasResults) {
              console.log(
                "✅ Backend results received! Storing and navigating..."
              );

              // Store results in sessionStorage if needed
              if (data.shouldStoreInClient) {
                const { storeThreeTierResults } = await import(
                  "@/utils/threetierResults"
                );
  ...
  const startScan = async () => {
    ...
    try {
      ...
      console.log(
        ...
        process.env.NEXT_PUBLIC_SCANNER_BASE_URL
      );
      ...
      if (participantData) {
        ...
        console.log(
          "Sending participant data to scanner with session ID:",
          sessionId,
          participantData
        );
      }
      ...
      if (response.data.success) {
        ...
        if (response.data.data.processing_status === "sent_to_backend") {
          console.log(
            "✅ Fingerprint captured and sent to backend for processing"
          );
          console.log(
            "🔄 Backend will send results directly to frontend callback"
          );
          ...
          console.log(
            ...
            response.data.data.backend_error
          );
        } else {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to scan fingerprint"
      );
    } finally {
      setScanning(false);
    }
